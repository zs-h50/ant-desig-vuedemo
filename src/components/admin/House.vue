<template>
	<a-table :columns="columns" :data-source="dataSource" :scroll="{ x: 1650, y: 385 }" :pagination="paginationOpt" row-key="hId">
		...
		<a-modal title="修改" :visible="visible" :footer="null" @cancel="handleCancel">
			<a-form :form="upform" :label-col="{ span: 5 }" :wrapper-col="{ span: 12 }" @submit="houseSubmit">
				<a-form-item label="学生标识">
				...
				<a-form-item v-model:value="upform.hRemark" label="备注">
					<a-input v-decorator="['hRemark', { rules: [{ required: false}]}]"
						placeholder="请输入备注" />
				</a-form-item>
				<a-form-item :wrapper-col="{ span: 12, offset: 5 }">
